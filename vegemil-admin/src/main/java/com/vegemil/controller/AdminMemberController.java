...
@Controller
public class AdminMemberController extends UiUtils {

	//@Autowired
	//private AdminBabyService adminBabyService;
	
	@RequestMapping(value = "/admin/member/{viewName}")
    public String adminMember(@PathVariable(value = "viewName", required = false) String viewName)throws Exception{
		// View attribute
		return "admin/member/"+viewName;
    }
	
	@RequestMapping(value = "/admin/member/mailSend/send", method = {RequestMethod.GET, RequestMethod.POST})
	public String memberMailSend(final  MailDTO params, Model model) {

		MailDTO mailDto = new MailDTO();
		try {
			mailDto.setTitle(params.getTitle());
			mailDto.setMessage(params.getMessage());
			mailDto.setAddress(params.getAddress());
		} catch (DataAccessException e) {
			return showMessageWithRedirect("데이터베이스 처리 과정에 문제가 발생하였습니다.", "/admin/member/mailSend", Method.GET, null, model);

		} catch (Exception e) {
			return showMessageWithRedirect("시스템에 문제가 발생하였습니다.", "/admin/member/mailSend", Method.GET, null, model);
		}

		return showMessageWithRedirect("고객상담 등록이 완료되었습니다.", "/admin/member/mailSend", Method.GET, null, model);
	}
